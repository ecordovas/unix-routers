...
#define PACKAGE "dial-repeater"
#define MDNS_ADDR "239.255.255.250"
...
static ssize_t send_packet_back(int fd, const void *data, size_t len, in_addr_t addr, uint16_t port) {
...
static void show_help(const char *progname) {
	fprintf(stderr, "mDNS repeater (version Edgar)\n");
...
struct sockaddr_in srv;
		...
		srv.sin_port = htons(35260);
		srv.sin_addr.s_addr = inet_addr("192.168.1.1");
r = -1;
if ((r = bind(s, (struct sockaddr *)&srv, sizeof(srv))) < 0) {
		log_message(LOG_ERR, "send bind(): %m");
	}
...
struct sockaddr_in srv2;
		...
		bzero(&srv2, sizeof(srv));
		srv2.sin_family = AF_INET;
		srv2.sin_port = htons(35261);
		srv2.sin_addr.s_addr = inet_addr("192.168.1.1");
r = -1;
if ((r = bind(s2, (struct sockaddr *)&srv2, sizeof(srv2))) < 0) {
		log_message(LOG_ERR, "send bind(): %m");
	}
	...
	while (! shutdown_flag) {
		...
		if (FD_ISSET(server_sockfd, &sockfd_set)) {
			...
			ssize_t recvsize = recvfrom(server_sockfd, res, sizeof(res)-1, 0, 
			...
			char buf[] = "HTTP/1.1 200 OK\r\n"\
					"USN: uuid:9de5e53a-d7c0-4a4e-a16b-2fa5e7d03a12::urn:dial-multiscreen-org:service:dial:1\r\n"\
					"CACHE-CONTROL: max-age=1800\r\n"\
					"EXT: \r\n"\
					"ST: urn:dial-multiscreen-org:service:dial:1\r\n"\
					"LOCATION: http://192.168.1.11:34996/upnp/dev/9de5e53a-d7c0-4a4e-a16b-2fa5e7d03a12/desc.xml\r\n"\
			...
			char *orig_addr_str = strdup(inet_ntoa(fromaddr.sin_addr));
			if (foreground)
				printf("\r\n\r\nsending response to %s : %d\n", orig_addr_str, ntohs(fromaddr.sin_port));

			ssize_t sentsize = send_packet_back(s, buf, strlen(buf), fromaddr.sin_addr.s_addr, fromaddr.sin_port);

			if (foreground)
				printf("\r\n\r\ncopying response to %s : %hu\n", "192.168.1.11", ntohs(fromaddr.sin_port));

			send_packet_back(s2, buf2, strlen(buf2), inet_addr("192.168.1.11"), fromaddr.sin_port);
